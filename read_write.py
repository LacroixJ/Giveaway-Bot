...
def check_recent():
    ...
    for x in giveaway_list:
        if len(x[6].split(sep="/")) >= 3 and len(x[8].split(sep=":")) >= 2:
            ...
            if (biggest_date - date).total_seconds() < 0 and (
                    date - datetime.datetime.utcnow()).total_seconds() <= 10:
                biggest_date = date
                giveaway_id = x[0]
...
def store_giveaway(giveaway):
    ...
    if mode_update == 0:
        ...
        values = (giveaway.get_id(), giveaway.get_header(),
                  giveaway.get_description(), giveaway.get_image(),
                  giveaway.get_number_of_winners(), giveaway.timeframe.start,
                  giveaway.timeframe.end, giveaway.timeframe.starttime,
                  giveaway.timeframe.endtime, giveaway.get_status())
        cursor.execute(sql, values)
        database.commit()
        return
    elif mode_update == 1:
        ...
        WHERE giveaway_id=%s"""
        variables = (giveaway.get_header(), giveaway.get_description(),
                     giveaway.get_image(), giveaway.get_number_of_winners(),
                     giveaway.timeframe.start, giveaway.timeframe.end,
                     giveaway.timeframe.starttime, giveaway.timeframe.endtime,
                     giveaway.get_status(), giveaway.get_id())
    ...
    if insertbool:
        ...
        values = (giveaway.get_id(), x)
        cursor.execute(sql, values)
        print("inserted " + x + " into entrants for giveaway " +
              giveaway.get_id())
        database.commit()
